...
class HCCLGroup(BaseGroup):
    ...
    def broadcast(
        ...
        input_tensor, output_tensor = self._get_single_tensor_pair(tensor, tensor)
    ...
    def allgather(
        ...
        input_tensor, output_tensor = self._get_single_tensor_pair(
    ...
    def _get_single_tensor_pair(
        self,
        input_tensors: Any,
        output_tensors: Any,
    ) -> Tuple[torch.Tensor, torch.Tensor]:
        """Validate single-tensor inputs and return the (input, output) pair.

        This helper accepts either a tensor or a single-element list for input/output.
        It enforces the single-device/single-tensor constraint and checks that the
        tensor device matches the communicator's initialization device.
        """
        ...
        if isinstance(input_tensors, list):
            if len(input_tensors) != 1:
                raise RuntimeError(
                    "HCCLGroup now supports a single device per process for collective ops."
                )
            input_tensor = input_tensors[0]
        if isinstance(output_tensors, list):
            if len(output_tensors) != 1:
                raise RuntimeError(
                    "HCCLGroup now supports a single output tensor for collective ops."
                )
        ...
        device = get_tensor_device(input_tensor)
    ...
    def _point2point(
        ...
        """Execute a point-to-point op using the shared communicator."""
        if not isinstance(tensor, torch.Tensor):
            raise RuntimeError("Point-to-point ops require a single torch.Tensor.")
